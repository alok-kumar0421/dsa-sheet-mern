...
router.post("/:id/toggle", protect, async (req, res) => {
  try {
    ...
    const existing = user.progress.find(p => p.problem.toString() === problemId);
  ...
  try {
    ...
    problems.forEach(p => {
      ...
      const solved = user.progress.find(
        x => x.completed && x.problem.toString() === p._id.toString()
      );

      if (solved) chapters[p.chapter].solved++;
    });
    ...
    const summary = Object.entries(chapters).map(([chapter, data]) => ({
      ...
      solved: data.solved,
      status:
        data.solved === 0 ? "Not started" :
        data.solved === data.total ? "Completed" : "In progress"
    }));
